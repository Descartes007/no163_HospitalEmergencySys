...
public class DictionaryServletContextListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        try {
            ...
            if (appContext == null) {
                logger.warn("Spring ApplicationContext 尚未准备好，跳过字典表初始化（contextInitialized）");
                return;
            }
            ...
            try {
                ...
                logger.warn("获取 dictionaryService bean 失败，跳过字典表初始化：{}", ex.getMessage());
            }
            ...
            logger.error("DictionaryServletContextListener 初始化时发生异常，已捕获并忽略，防止阻塞容器启动", t);
        }
    }
